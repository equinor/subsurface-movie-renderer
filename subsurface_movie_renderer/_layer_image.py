...
# Functions to Plot
def create_plot(prodDF, end, index_to_plot=[], plot_size=(10,5),ticks_axis='X', font=None):
    '''
...
def add_layer(configuration,tmp_dir_path):
    ...
    index_to_plot = []
    index_for_ticks =[date_index[abs(pd.Series(date_selection) - year_initial[0]).idxmin()+1]] 
    ...
    with tqdm(
        ...
        for i,end in enumerate(date_index):
            ...
            text = 'SLEIPNER'
            draw.text((50, 50), text, font = font, fill='black', align ="left",stroke_width=1,stroke_fill="black")
            ...
            fig, ax = create_plot(prodDF, end, index_for_ticks, plot_size=(width/(100*3), height/(100*4)),ticks_axis='X', font=prop)
            ...
            canvas.paste(img, (width-wi, height-he, width , height), img) # left, upper, right, and lower 
